...
def sanitize_text(text, max_len=200):
    if not text: return ""
    return str(escape(str(text).strip()[:max_len]))
...
def api_pincode(pin):
    if not re.match(r'^\d{6}$', pin):
        return jsonify([{'Status': 'Error', 'Message': 'Invalid pincode format'}]), 400
...
def profile():
    ...
    if request.method == 'POST':
        new_username      = sanitize_text(request.form.get('username', user.get('username')), 50)
        new_email         = request.form.get('email', user.get('email')).strip()
        delivery_name     = sanitize_text(request.form.get('delivery_name', ''), 100)
        mobile            = request.form.get('mobile', '').strip()
        delivery_pincode  = request.form.get('delivery_pincode', '').strip()
        delivery_state    = sanitize_text(request.form.get('delivery_state', ''), 50)
        delivery_district = sanitize_text(request.form.get('delivery_district', ''), 50)
        delivery_taluka   = sanitize_text(request.form.get('delivery_taluka', ''), 50)
        delivery_village  = sanitize_text(request.form.get('delivery_village', ''), 50)
        delivery_street   = sanitize_text(request.form.get('delivery_street', ''), 200)
...
def place_order():
    ...
    try:
        ...
        for item in items_data:
            item['name'] = sanitize_text(item.get('name', 'Unknown'), 100)
            try:
                qty_num = int(item.get('qty', 1))
                item['qty'] = max(1, min(qty_num, 999))
            except ValueError:
                item['qty'] = 1

        try:
            total_amount = float(data.get('total', 0))
            if total_amount < 0 or total_amount > 999999:
...
def shops_by_distance():
    ...
    try:
        ...
        if not is_valid_coordinates(user_lat, user_lng):
...
def update_uploaded_order():
    ...
    try:
        ...
        for it in updated_items:
            ...
            it['price'] = max(0.0, price)
            it['qty'] = max(1, min(qty_num, 999))
            it['qty_str'] = qty_str
            it['name'] = sanitize_text(it.get('name', 'Unknown'), 100)
